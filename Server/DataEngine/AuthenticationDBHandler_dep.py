...
class AuthenticationSQLDBHandler(BaseLogging):
    ...
    def add_api_role(self, username = None, roles = None):
        ...
        ## Turns the list into a str such as: "filehsot_admin,iam_admin"
        print(joined_roles)
        try:
            roles_query = f'UPDATE api_users SET role = ? WHERE username = ?'
            values = (joined_roles,username,)
            self.cursor.execute(roles_query, values)
            self.dbconn.commit()
            return True
