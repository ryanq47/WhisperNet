...
class ListenerHttpCommandSync:
    ...
    def handle_data(self, data):
        ...
        for client_command in data:
            ...
            client_nickname = client_command['client_nickname']
            ...
            if self.Data.Clients.check_if_client_exists(client_name = client_nickname):
                self.logger.debug(f"Client object found for {client_nickname}.")
                client_object = self.Data.Clients.get_client_object(client_name = client_nickname)
            ...
            # it COULD happen though *if* the server sends data with a client that isn't in this listener. 
                ...
                self.logger.warning(f"Queueing command for client obejct that does not exist: {client_nickname}. Creating object.")
                client_object = Client(nickname=client_nickname)
            ...
            self.logger.debug(f"Queueing command for {client_nickname}")
            client_object.enqueue_command(command = client_command)
            self.logger.debug(f"Next queued command for {client_nickname} is: {client_object.peek_next_command()} ")
