...
int main() {
    ...
    while ( 1 == 1) {
        ...
        if ( strcmp(ph_value, "continue" ) == 0 ) {
            printf("continue called, sleeping in main\n");
...
char * phone_home(char * client_id) {
    ...
    if(inet_pton(AF_INET, server_connection.address, &serv_addr.sin_addr)<=0) { //does some conversion, not 100% the logic behind it
