...
class SpawnListenerPopup(QDialog):
    def __init__(self, parent=None):
        ...
        self.ui_file = loader.load('QtComponents/Listeners/spawn_listener_form_popup.ui', self)
    ...
    def listener_decision_tree(self):
        ...
        if self.listener_type.currentText().lower() == "http":
            self.spawn_http_listener()

        else:
            self.logger.error(f'Somehow selected a listener type that did not exist, type selected: {self.listener_type.lower()}')
...
class KillListenerPopup(QDialog):
    def __init__(self, parent=None):
        ...
        self.data = Data()
        ## This NEEDS self as a second arg for some reason.
        self.ui_file = loader.load('QtComponents/Listeners/kill_listener_form_popup.ui', self)
    ...
    def __ui_load(self):
        ...
        try:
            ...
            self.listener_type = self.ui_file.findChild(QComboBox, "listener_popup_type_combo")
            self.listener_spawn_button = self.ui_file.findChild(QPushButton, "listener_popup_kill_button")
            # Dec tree for type - figure out where to put this based on type input.
            self.listener_spawn_button.clicked.connect(self.listener_decision_tree)
    ...
    def listener_decision_tree(self):
        ...
        if self.listener_type.currentText().lower() == "http":
            self.kill_http_listener()

        else:
            self.logger.error(f'Somehow selected a listener type that did not exist, type selected: {self.listener_type.lower()}')

    def kill_http_listener(self):
        """
            Sends a network request to the Server, to kill the listener

            Potential improvements:
             - Pull listener data automatically from the data singleton, removing the "type" selector for the user. 
        ...
        self.logger.info(f'Sending request to kill HTTP listener {listener_params["nickname"]}')
