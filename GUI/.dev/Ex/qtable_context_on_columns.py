import sys
from PySide6.QtWidgets import QApplication, QTableWidget, QTableWidgetItem, QMenu
from PySide6.QtCore import Qt
from PySide6.QtGui import QMouseEvent

class CustomTableWidget(QTableWidget):
    def __init__(self, rows, columns):
        super().__init__(rows, columns)

        # Sample data
        for i in range(rows):
            for j in range(columns):
                self.setItem(i, j, QTableWidgetItem(f"Item {i},{j}"))
        self.setVerticalHeaderLabels([f"Row {i}" for i in range(rows)])

    def mousePressEvent(self, event: QMouseEvent):
        if event.button() == Qt.RightButton:
            header = self.verticalHeader()
            rowIndex = header.logicalIndexAt(event.pos())
            if rowIndex != -1:  # Check if the click is on a row header
                self.showHeaderContextMenu(rowIndex, event.globalPos())

    def showHeaderContextMenu(self, rowIndex, globalPos):
        menu = QMenu(self)
        action1 = menu.addAction("Action 1")
        action2 = menu.addAction("Action 2")
        action3 = menu.addAction("Action 3")

        action = menu.exec(globalPos)

        if action == action1:
            print(f"Action 1 selected on row {rowIndex}")
        elif action == action2:
            print(f"Action 2 selected on row {rowIndex}")
        elif action == action3:
            print(f"Action 3 selected on row {rowIndex}")

app = QApplication(sys.argv)
table = CustomTableWidget(5, 3)  # 5 rows, 3 columns
table.show()
sys.exit(app.exec())
